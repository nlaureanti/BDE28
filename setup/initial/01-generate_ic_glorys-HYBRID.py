...
import sys
sys.path.append('/home/nicole.laureanti/HCtFlood')
...
def write_initial(glorys_file, vgrid_file, grid_file, output_file):
    ...
    vgrid = xarray.open_dataset(vgrid_file)['dz']
...
#    glorys.time.data[0] = np.datetime64(f'{year}-12-09T00:00:00.000000000')
    ...
    interped_t = glorys_to_t(flooded[['thetao', 'so', 'zos']]).rename({'lon':'xh','lat':'yh'})#.drop(['lon', 'lat'])
    ...
    uo = urot.isel(nxp=slice(0, None, 2), nyp=slice(1, None, 2)).rename({'lon':'xq','lat':'yh'})#.rename({'nxp': 'xq', 'nyp': 'yh'})
    uo.name = 'uo'
    vo = vrot.isel(nxp=slice(1, None, 2), nyp=slice(0, None, 2)).rename({'lon':'xh','lat':'yq'})#.rename({'nxp': 'xh', 'nyp': 'yq'})
    vo.name = 'vo'
    #print(uo,vo,interped_t)
    interped = (
    ...
    #nlaureanti added on FATAL from PE    54: error finding variable xh in file INPUT/glorys_ic_75z_1997.nc in hinterp_extrap
    for vv in list(interped.coords.keys()):
        interped[vv] = xarray.DataArray(interped[vv].data, dims=[vv], coords={vv:(vv,interped[vv].data) } , name=vv)
...
def main():
    global date
    date=np.datetime64('2022-01-21')
    glorys_file = f'/scratch/servicos/inpe2//nicole.laureanti/data/glorys/glorys_{date}.nc'

    vgrid_file = '/scratch/servicos/inpe2//nicole.laureanti/BDE28/exp.002/INPUT/hycom1_75_800m.nc'

    grid_file = '/scratch/servicos/inpe2//nicole.laureanti/BDE28/grid/ocean_hgrid.nc'
    output_file = f'/scratch/servicos/inpe2//nicole.laureanti/BDE28/INPUT/glorys_ic_75h_{date}.nc'
