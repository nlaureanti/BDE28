...
def write_runoff(glofas, glofas_mask, hgrid, coast_mask, out_file):
    ...
    latb = hgrid.y[::2, ::2]
    # From Alistair
    print(glofas_lonb.shape,glofas.longitude.shape)
    ...
    glofas_area = dx * dy
    glofas_kg =  glofas * 1000  / glofas_area

    plt.figure(figsize=[12,12])
    ax=plt.axes()
    glofas_area.plot(ax=ax)
#    plt.savefig(f'area_{y}.png')

    plt.figure(figsize=[12,12])
    ax=plt.axes()
    glofas_kg.isel(time=0).plot(ax=ax,cmap='Blues', vmax=0.02, vmin=0.0)
    #plt.savefig(f'init_glofas_kgsm_t0_{y}.png')

    plt.figure(figsize=[12,12])
    ax=plt.axes()
    (glofas/glofas_area).isel(time=0).plot(ax=ax,cmap='Blues', vmax=0.02, vmin=0.0)
#    plt.savefig(f'glofas_m3sm_t0_{y}.png')
    ...
    #glofas_regridded = glofas_to_mom_con(glofas_kg.fillna(0.0))
    ...
    #plt.savefig(f'final_glofas_{y}.png')
...
#    plt.savefig('uparea.png')
    ...
    window = 1  # look with +- this number of grid points
    ni, nj = uparea.shape
    ...
    for y in range(1997, 2017):
        ...
        glofas = (
            xarray.open_mfdataset(files, combine='by_coords',data_vars=['dis24'], 
                    combine_attrs="drop_conflicts")
            .sel(time=slice(f'{y-1}-12-30 00:00:00', f'{y+1}-01-02 00:00:00'), **glofas_subset)
            .sel( **glofas_subset)
            .dis24
        )
        plt.figure(figsize=[12,12])
        ...
        #plt.savefig(f'glofas_m3s_{y}.png')
        #out_file = f'/home/nicole/workdir/SWA14/INPUT/glofas_runoff_{y}.nc'
        out_file = f'/home/nicole/workdir/SWA14/setup/rivers/new_window_w1x1000/glofas_runoff_{y}.nc'
