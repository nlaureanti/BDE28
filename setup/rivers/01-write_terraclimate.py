...
def write_runoff(glofas, glofas_mask, hgrid, coast_mask, out_file):
    ...
    glofas_latb = np.linspace(glofas['lat'][0]+.021, glofas['lat'][-1]-.021, glofas.lat.shape[0]+1)
    glofas_lonb = np.linspace(glofas['lon'][0]-.021, glofas['lon'][-1]+.021, glofas.lon.shape[0]+1)
    ...
    dlon = dlat = 0.041  # GloFAS grid spacing
...
if __name__ == '__main__':
    coast = get_coast_mask('/scratch/servicos/inpe2/nicole.laureanti/SWA14/grid/ocean_mask.nc')
    hgrid = xarray.open_dataset('/scratch/servicos/inpe2/nicole.laureanti/SWA14/grid/ocean_hgrid.nc')

    # For NWA: subset GloFAS to a smaller region containing NWA.
    glofas_subset = dict(lat=slice(5,-55), lon=slice(-69, -9))
    ...
    uparea = xarray.open_dataarray('/scratch/servicos/inpe2/nicole.laureanti/data/GloFAS/agg_terraclimate_q_1958_CurrentYear_GLOBE.nc').sel(**glofas_subset)
    uparea=xarray.DataArray( np.full(uparea[0].shape,16e6),
                             dims=('lat','lon'),
                            coords={
                                    'lon':('lon',uparea.lon.values),
                                   'lat':('lat',uparea.lat.values)}
                            )
    ...
    window = 5  # look with +- this number of grid points
    ni, nj = uparea.shape
    ...
    for y in range(1):
        print(y)
        files='/scratch/servicos/inpe2/nicole.laureanti/data/GloFAS/agg_terraclimate_q_1958_CurrentYear_GLOBE.nc'
        glofas =    xarray.open_mfdataset(files, combine='by_coords',data_vars=['dis24'], 
                combine_attrs="drop_conflicts").sel(**glofas_subset).q
        ...
        out_file = f'/scratch/servicos/inpe2/nicole.laureanti/SWA14/forcing_final_ERA5/terra_runoff.nc'
